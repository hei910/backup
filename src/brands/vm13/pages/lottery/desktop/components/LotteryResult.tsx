...
const LotteryResult = ({ lotoResult, color, shape }: LotteryResultProps) => {
    ...
    if (!lotoResult || lotoResult.length <= 1) return <SEmptyResultText>{t('lottery.empty')}</SEmptyResultText>

    return (
        <SLotteryResult>
            {lotoResult?.map((result: any, i: number) => (
                <SGameBall color={getLotoColor(+lotoResult[i], color)} isCircle={shape === 'circle'}>
                    {result}
                </SGameBall>
            ))}
        </SLotteryResult>
