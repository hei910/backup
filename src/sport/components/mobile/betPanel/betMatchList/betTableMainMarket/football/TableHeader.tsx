...
const TeamHeader: React.FC<TableTeamHeaderHeaderProps> = ({ match, matchInfo, event }) => {
    ...
    const getLiveStatusText = (liveStatus: string) => {
        //source A = 1h while source B = 1H
        switch (liveStatus.toUpperCase()) {
            case '1H':
                return t('liveStatus.football.1h')
            case '2H':
                return t('liveStatus.football.2h')
            case 'HT':
                return t('liveStatus.football.ht')
            default:
                return ''
        }
    }
    ...
    return (
        <SHeaderLayout>
            <SHeaderLeft>
                ...
                <SMoreLayout>
                    <MoreMarketBadge
                        count={matchInfo.totalMarkets}
                        matchId={matchInfo.matchId}
                        matchStatus={copiedMatchStatus}
                    />
                </SMoreLayout>
            </SHeaderLeft>
            <SHeaderRight>
                ...
                {date === 'inplay' && (
                    <Link
                        to={{
                            pathname: `/sport/${date}/football/details/${matchInfo.matchId}/${matchInfo.source}`,
