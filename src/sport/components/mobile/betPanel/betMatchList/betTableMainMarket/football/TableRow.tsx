...
// import { Fixture } from 'stores/data/types';
...
const TableRow: React.FC<TableRowProps> = ({ match }) => {
    ...
    const getOddsButton = (event: ConvertedEvent, market?: ConvertedMarket, outcomeCode?: string) => {
        ...
        if (market.marketCode.startsWith('1x2')) {
    ...
    const getOddsData = (event: ConvertedEvent, isHalfView: boolean) => {
        ...
        const hadMarket = getMarket(event, '1x2')
        const ahMarket = getMarket(event, 'ah')
        const ouMarket = getMarket(event, 'ou')
        const had1stMarket = getMarket(event, '1x21st')
        ...
        return (
            <>
                <SOddsColumn>
                    ...
                    {getOddsButton(event, isHalfView ? had1stMarket : hadMarket, 'd')}
                ...
                <SOddsColumn isHidden={!isTabletLayout}>
                    ...
                    {getOddsButton(event, had1stMarket, 'd')}
            ...
            {events.map((event) => {
                return (
                    <div key={event.ctid + event.fixtureId}>
                        <TableHeader match={match} matchInfo={matchInfo} event={event} />
                        <SMatchLayout>
                            <SCompetitorLayout>
                                <SCompetitorItem>
